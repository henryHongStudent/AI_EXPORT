...
const connectOpenAI = async (pdfUrl, imageName) => {
  console.log("==== connectOpenAI 진입 ====");
  console.log("OpenAI Key 존재 여부:", !!process.env.OPENAI_API_KEY);
  console.log("요청할 이미지 URL:", pdfUrl);
  ...
  try {
    ...
    if (
      ...
      throw new Error("OpenAI 응답이 비어있거나 올바르지 않습니다.");
    }
    ...
    // error가 객체면 message만, 아니면 전체 반환
    throw new Error(error?.message || JSON.stringify(error));
  }
};

console.log("Lambda 시작됨, OPENAI_API_KEY:", process.env.OPENAI_API_KEY);

export const handler = async (event) => {
  console.log("==== Lambda handler 진입 ====");
  try {
    ...
    const results = await Promise.all(
      files.map(async (file, idx) => {
        console.log(`Processing file[${idx}]:`, JSON.stringify(file));
        if (!file) {
          console.error(`file[${idx}] is undefined!`);
          return { name: "undefined", error: "file is undefined" };
        }
        const { imageName, pdfUrl } = file;
        try {
          const openAIResult = await connectOpenAI(pdfUrl, imageName);
          return {
            name: imageName,
            data: openAIResult,
          };
        } catch (error) {
          console.error(`Error processing file ${imageName}:`, error);
          return {
            name: imageName,
            error: error.message,
          };
